...
::: {.reply}
Thank the reviewer for this thoughtful comment.
...
::: {.reply}
We thank the reviewer for this comment and acknowledge that we mixed the two separate concepts of "outlier studies" and "studies with uncertain estimates".
With the statement quoted above, we intended to discuss only how "studies with uncertain estimates" are affected by shrinkage.

To address the reviewers comment, we have therefore removed all references to "outlier studies" and replaced this with "studies reporting uncertain estimates" or comparable wording.
...
"Varying the prior on the location of $\mu$ while holding $\tau$ at $\mathcal{N}^+(0, 2)$, we found neither skeptical and enthusiastic priors on the location of $\mu$ substantially alter our conclusions.
Varying the strength of shrinkage by decreasing or increasing the scale of $\tau$ while holding $\mu$ at $\mathcal{N}(0, 100)$ also did not substantially alter what we concluded from the estimates.
...
In contrast, frequentist models rely on inverse-variance weighting that is fully determined by the data – studies with large uncertainty are weighed less, but the degree to which this happens cannot be configured by the researcher.
...
This has lead to the apparent asymmetry between $\tau^2$ and $I^2$ summaries that we thank the reviewer for pointing out.
...
An $I^2$ calculated this way would have been consistent with the reviewers intuition, resulting in an estimated $I^2$ of 86.6%, larger than the frequentist $I^2$ of 85.0%.
...
"In the Bayesian model, between-study heterogeneity was estimated as a posterior median $\tau^2$ of 3.3 (95% CrI: 1.0 to 9.5), corresponding to an $I^2$ of 84.7% (95% CrI: 63.9% to 94.2%)."
:::
...
::: {.reply}
We thank the reviewer for raising this. The GitHub repository is now publicly accessible at [github.com/PaulinaSell/lead-IQ_meta-analysis](https://github.com/PaulinaSell/lead-IQ_meta-analysis).
:::
...
Furthermore, we thank the reviewer for pointing out that the sample size in our study warrants a small-sample adjustment.
...
Regarding the reviewers third point, we acknowledge that the claims we make regarding outlier treatment in the Bayesian model are too strong, and thank them for this comment; we believe that with its help, we could significantly improve the clarity and argumentation in our manuscript.
Since reviewer 1 has raised a similar point, we omit repeating our reply and the associated changes here, and would instead gently refer the reviewer to our reply to reviewer 1's comment 2.
:::
...
::: {.reply}
We thank the reviewer for pointing out these numerical inconsistencies. We have fixed these issues in the Graphical Abstract, the Abstract, Results, Figure 2 and Table 4.
:::
...
::: {.reply}
We thank the reviewer for raising this — the prose description did not fully match the PECOTS criteria in Table 1.
...
::: {.reply}
We thank the reviewer for catching this. The upper limit indeed was wrong: a coding error caused the point estimate of $\tau^2$ to be printed in place of the upper confidence limit. We have corrected this, and the confidence interval is now reported from the correct bound returned by `metafor`.
:::
