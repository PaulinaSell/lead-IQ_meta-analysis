...
# main predictive intervals (assuming trunc normal for tau)
tau_alt <- make_dist(
  "normal",
  mean = tau_hat,
  std = se_tau,
...
tau_samples <- tau_alt$r(n_samples)
...
# alternative predictive intervals (assuming log-normal for tau)
log_tau_mu <- log(tau_hat)
log_tau_sd <- se_tau / tau_hat
tau_alt_samples <- exp(rnorm(n_samples, log_tau_mu, log_tau_sd))
theta_new <- rnorm(n_samples, mu_samples, tau_alt_samples)

theta_new_mean <- mean(theta_new)
theta_new_q025_alt <- quantile(theta_new, probs = 0.025)
theta_new_q975_alt <- quantile(theta_new, probs = 0.975)
