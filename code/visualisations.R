...
table_all <- readRDS(
  "/Users/paulinasell/Documents/UBA/PARC/Metaanalysis_lead_IQloss/RProj/manuscript/tables_figures/main/table_sensi.rds"
)
...
# Identify row indices for styling
# Identify row indices for styling
first_model_rows <- which(table5_gt$model_display != "")
bold_rows <- which(table5_gt$is_bold)
...
table5_gt %>%
  ...
  tab_caption(
    caption = "Tab. 5: Results of sensitivity analyses by modeling framework (Bayesian vs. frequentist). RoB, risk of bias; UI, uncertainty interval (credible interval for Bayesian and confidence interval for frequentist model results)."
  )
